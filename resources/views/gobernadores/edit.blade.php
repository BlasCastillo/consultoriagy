        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-slate-200">
                <div class="p-6 text-slate-900">
                    <form action="{{ route('gobernadores.update', $gobernador->id) }}" method="POST" autocomplete="off">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-4">
                            ...
                            <select name="titulo_id" id="titulo_id" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-800 focus:ring-slate-800 sm:text-sm" required>
                                ...
                                @foreach($titulos as $titulo)
                                    <option value="{{ $titulo->id }}" {{ $gobernador->titulo_id == $titulo->id ? 'selected' : '' }}>{{ $titulo->abreviatura }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="nombres" id="nombres" value="{{ $gobernador->nombres }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-800 focus:ring-slate-800 sm:text-sm" required>
                        </div>
                        
                        <div class="mb-4">
                            <label for="apellidos" class="block text-sm font-medium text-slate-700">Apellidos</label>
                            <input type="text" name="apellidos" id="apellidos" value="{{ $gobernador->apellidos }}" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-slate-800 focus:ring-slate-800 sm:text-sm" required>
                        </div>
                        
                        <div class="mb-6 flex items-center">
                            <input type="checkbox" name="estado" id="estado" value="1" {{ $gobernador->estado ? 'checked' : '' }} class="h-4 w-4 text-slate-800 border-slate-300 rounded focus:ring-slate-800">
