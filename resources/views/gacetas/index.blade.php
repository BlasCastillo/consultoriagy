            ...
            <div class="bg-white p-6 rounded-lg shadow-sm border border-slate-200 mb-6">
                ...
                <form method="GET" action="{{ route('gacetas.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" name="anio" value="{{ request('anio') }}"
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-slate-800 focus:ring-slate-800 sm:text-sm">
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end space-x-3 mt-4">
                        {{-- Botón Limpiar (Cancelar/Secundario) --}}
                    ...
                    <table class="min-w-full">
                        <thead class="bg-slate-800 text-white">
                            <tr>
                                ...
                                @endif
                                <th class="py-3 px-4 text-right text-sm font-semibold uppercase tracking-wider">Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($gacetas as $g)
                                <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="py-3 px-4 text-sm text-slate-600">{{ $g->tipo }}</td>
                                    @if(!auth()->user()->hasAnyRole(['Institucion', 'Institucional']))
                                            ...
                                            @if(auth()->user()->hasAnyRole(['Jefe de Digitalización', 'Super Admin', 'Super Administrador']) || auth()->user()->roles->count() === 0)
                                                @if($g->estado === 'Solicitada')
                                                    ...
                                                    <a href="{{ route('gacetas.checklist', $g->id) }}"
                                                        class="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 rounded-lg shadow-sm transition-all duration-300 flex items-center justify-center gap-2 font-medium text-sm md:text-base" title="Validar">
                                                ...
                                                @elseif($g->estado === 'Reservada')
                                                    ...
                                                    <a href="{{ route('gacetas.asignar', $g->id) }}"
                                                        class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2.5 rounded-lg shadow-sm transition-all duration-300 flex items-center justify-center gap-2 font-medium text-sm md:text-base" title="Asignar">
                                                ...
                                                @elseif($g->estado === 'Por Aprobar')
                                                    ...
                                                    <a href="{{ route('gacetas.aprobar', $g->id) }}"
                                                        class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-lg shadow-sm transition-all duration-300 flex items-center justify-center gap-2 font-medium text-sm md:text-base" title="Revisar">
                                            ...
                                            @if(auth()->user()->hasAnyRole(['Digitalizador', 'Super Admin', 'Super Administrador']) || auth()->user()->roles->count() === 0)
                                                @if($g->estado === 'En Escaneo')
                                                    ...
                                                    <a href="{{ route('gacetas.upload_pdf', $g->id) }}"
                                                        class="bg-slate-800 hover:bg-slate-900 text-white px-4 py-2.5 rounded-lg shadow-sm transition-all duration-300 flex items-center justify-center gap-2 font-medium text-sm md:text-base" title="Subir PDF">
                            ...
                            @empty
                                <tr>
                                    <td colspan="{{ auth()->user()->hasAnyRole(['Institucion', 'Institucional']) ? 4 : 6 }}"
                                        class="py-6 text-center text-sm text-slate-500">No se encontraron gacetas.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
